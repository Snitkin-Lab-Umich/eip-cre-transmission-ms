...
# plasmid clusters
mob_plasmids <- read_rds('data/mobtyper/CRE_EIP_mobtyper_mash_nn_plasmid_matrix_050923.RDS')
mob_clusters <- read_rds('data/mobtyper/CRE_EIP_mobtyper_plasmid_cluster_matrix_050923.RDS')
mob_results <- read_rds('data/mobtyper/CRE_EIP_mobtyper_results_clean_KPC_050923.RDS')
# mob_contigs <- read_rds('data/mobtyper/CRE_EIP_mobtyper_contigs_clean_KPC_050923.RDS') 
# clinical factors
...
```{r}
n_distinct(facil_st$Patient_ID)
```

### Number of facilities

```{r}
n_distinct(facil_st$Facility_ID)
```
...
facil_st %>% 
  ...
  summarize(n_isolates = n(),
            n_facil = n_distinct(Facility_ID))
```
...
### Number of distinct STs by state and year

```{r}
fs1c <- facil_st_orig %>% 
  group_by(State, cult_year, Species, ST) %>% 
  filter(!is.na(cult_year)) %>% 
  summarize() %>% 
  group_by(State, cult_year, Species) %>% 
  count() %>% 
  mutate(Species = ifelse(grepl('hormaechei', Species), 'KPC-Eh', 'KPC-Kp')) %>% 
  ggplot(aes(x = cult_year, y = n, fill = Species)) +
  facet_grid(~State, scales = 'free', space = 'free') +
  geom_col() +
  scale_fill_manual(values = rev(c('coral2', 'aquamarine2'))) +
  labs(x = 'Year', y = 'Number of distinct STs')
```


```{r, fig.width=5.5, fig.height=7}
fs1 <- plot_grid(fs1a, fs1b + theme(legend.position = 'none'), fs1c, ncol = 1, labels = 'AUTO')

ggsave(plot = fs1, filename = 'figures/FigS1.png', width = 5.5, height = 7)
ggsave(plot = fs1, filename = 'figures/FigS1.tiff', width = 5.5, height = 7)
...
```{r}
# identifying phylogenetic transition events
...
keepers <- facil_st %>% 
  filter(!(duplicated(Patient_ID) & duplicated(ST) & duplicated(Facility_ID))) %>% 
  pull(Sample_ID)
...
```{r}
# plasmid info 
mob_plasmids_long <- mob_plasmids %>% 
  rownames_to_column(var = 'isolate_no') %>% 
  pivot_longer(!isolate_no, names_to = 'mash_nearest_neighbor', values_to = 'present') %>% 
  left_join(mob_results %>% 
              filter(!grepl('novel', primary_cluster_id)) %>% 
              select(primary_cluster_id, mash_nearest_neighbor) %>% distinct()) %>% 
  left_join(mob_results %>% 
              filter(!grepl('novel', primary_cluster_id)) %>% 
              select(isolate_no, primary_cluster_id, mash_nearest_neighbor, KPC_present)) %>% 
  mutate(state = gsub('_.*', '', isolate_no), .before = 2) %>% 
  group_by(mash_nearest_neighbor) %>% 
  filter(any(sum(KPC_present, na.rm = TRUE) > 0)) %>% 
  left_join(read_csv('data/isolate_data.csv') %>% 
  select(Sample_ID, Species, ST) %>% 
  rename(isolate_no = Sample_ID)) %>% 
  mutate(ST_orig = ST,
         ST = case_when(is.na(ST) ~ 'Other species',
                        ST %in% dominant_sts ~ ST, 
                        TRUE ~ 'Other')) %>% 
  ungroup()

cluster_info <- mob_plasmids_long %>% 
  filter(KPC_present == 1) %>%
  rename(plasmid_cluster = primary_cluster_id) %>% 
  inner_join(transitions %>% 
               mutate(phylo_cluster = paste0(state, ST, cluster, class2)) %>% 
               select(id, state, phylo_cluster, class2) %>% 
               rename(isolate_no = id)) %>% 
  select(isolate_no, state, ST, class2, phylo_cluster, plasmid_cluster) %>% 
  group_by(state, ST, phylo_cluster, plasmid_cluster) %>% 
  mutate(cluster_id = paste0(state, ST, phylo_cluster, plasmid_cluster),
         cluster_size = n()) %>% 
  ungroup()

nodes <- cluster_info %>%
               select(ST, state, class2, phylo_cluster, plasmid_cluster) %>%
               pivot_longer(!c(ST, class2, state), names_to = 'node_type', values_to = 'clust_id') %>%
  group_by(node_type, clust_id) %>% 
  summarize(n_in_clust = n()) %>% 
  ungroup() %>% 
  arrange(clust_id) %>% 
  left_join(cluster_info %>% select(ST, class2, phylo_cluster) %>% rename(clust_id = phylo_cluster) %>% distinct()) %>% 
  mutate(index = as.numeric(factor(clust_id)),
         node_type = ifelse(grepl('phylo', node_type), ST, 'Plasmid'),
         node_import = factor(ifelse(is.na(class2), 'Plasmid', as.character(class2)),
                              levels = c('Yes', 'No', 'Plasmid')))
  
edges <- cluster_info %>% 
  left_join(nodes %>% rename(phylo_cluster = clust_id) %>% rename(from = index) %>% select(-c(node_type, n_in_clust, ST, class2, node_import))) %>%
  left_join(nodes %>% rename(plasmid_cluster = clust_id) %>% rename(to = index) %>% select(-c(node_type, n_in_clust, ST, class2, node_import))) %>% 
  group_by(from, to, phylo_cluster, plasmid_cluster, state, ST, class2) %>% 
  summarize(n_in_grp = n())

clust_graph <- tbl_graph(directed = FALSE, 
             nodes = nodes,
             edges = edges)
```
...
```{r}
fs2d <- cluster_info %>%
  group_by(phylo_cluster) %>%
  filter(n() > 1) %>%
  group_by(state, ST, phylo_cluster, plasmid_cluster, cluster_size) %>% 
  summarize() %>% 
  ggplot(aes(x = phylo_cluster, y = cluster_size, group = plasmid_cluster, fill = ST)) +
  facet_grid(~state, scales = 'free', space = 'free') +
  geom_col(col = 'white') +
  scale_fill_manual(values = st_cols,
                    breaks = c('E. hormaechei ST171', 'E. hormaechei ST114',
  ...
  labs(x = 'Phylogenetic cluster', y = 'Number of isolates')
```


```{r, fig.width=8, fig.height=10}
fs2 <- plot_grid(plot_grid(fs2ab, fs2c, labels = c('', 'C'), label_x = -0.1),
                  fs2d, ncol = 1, labels = c('', 'D'), rel_heights = c(2, 0.5))
ggsave('figures/FigS2.png', width = 8, height = 10)
ggsave('figures/FigS2.tiff', width = 8, height = 10)
fs2
```
...
clust_cols <- st_cols[as.character(cluster_info$ST)]
names(clust_cols) <- cluster_info$cluster_id #paste0(cluster_info$state, cluster_info$ST, cluster_info$cluster)
...
```{r}
set.seed(2)
clust_graph_ct <- clust_graph %>%
  activate(edges) %>%
  filter(state == 'CT') %>% 
  activate(nodes) %>% 
  filter(centrality_degree() > 0) %>% 
  mutate(n_in_clust = centrality_degree(weights = n_in_grp)) %>% 
  ggraph(layout = 'fr') +
    geom_edge_link(aes(alpha = n_in_grp)) +
    geom_node_point(aes(col = node_type, size = n_in_clust)) +
  theme_graph() +
  scale_color_manual(values = c(st_cols, Plasmid = 'grey20'),
                    breaks = c('E. hormaechei ST171', 'E. hormaechei ST114',
                               'K. pneumoniae ST258', 'K. pneumoniae ST307',
                               'Other', 'Plasmid'), 
                    labels = c('KPC-Eh ST171', 'KPC-Eh ST114',
                               ...
                               'Other', 'Plasmid'),
                    drop = FALSE) +
  scale_edge_alpha_continuous(limits = c(0, 20), breaks = seq(5, 20, by = 5)) +
  scale_size_continuous(limits = c(0, 50), breaks = c(10, 20, 30), labels = c(10, 20, 30)) +
  labs(col = 'Cluster type', size = 'Number of isolates', edge_alpha = 'Number of\nshared isolates') +
  theme(title = element_text(size = 10))

clust_graph_mn <- clust_graph %>%
  activate(edges) %>%
  filter(state == 'MN') %>% 
  activate(nodes) %>% 
  filter(centrality_degree() > 0) %>% 
...
f1a <- plot_grid(plot_grid(clust_graph_ct + 
          ...
          clust_graph_tn + 
            theme(legend.position = 'none', 
                  plot.title = element_text(hjust = 0.5, size = 10, face = 'plain')) +
            labs(title = 'TN'), 
          ncol = 1), get_legend(clust_graph_mn + theme(legend.position = 'right')), nrow = 1) %>% 
  suppressWarnings()
```

```{r, fig.width=7, fig.height=4}
f1b <- cluster_info %>% 
  arrange(ST, -cluster_size) %>% 
  ggplot(aes(x = class2, 
             fill = factor(cluster_id, #paste0(state, ST, cluster), 
                           levels = unique(cluster_id)))) + #unique(paste0(state, ST, cluster))))) + 
...
# f1 <- plot_grid(f1a, plot_grid(f1b, NULL, nrow = 1, rel_widths = c(2, 0.5)), 
#                 ncol = 1, rel_heights = c(2, 1), labels = c('A', 'B'))

networks <- plot_grid(plot_grid(clust_graph_ct + 
                   theme(legend.position = 'none', 
                         plot.title = element_text(hjust = 0.5, size = 10, face = 'plain')) +
            labs(title = 'CT'), 
          clust_graph_mn + 
            theme(legend.position = 'none', 
                  plot.title = element_text(hjust = 0.5, size = 10, face = 'plain')) +
            labs(title = 'MN'), nrow = 1), 
          plot_grid(NULL,
          clust_graph_tn + 
            theme(legend.position = 'none', 
                  plot.title = element_text(hjust = 0.5, size = 10, face = 'plain')) +
            labs(title = 'TN'),
          NULL, nrow = 1, rel_widths = c(0.5, 1, 0.5)),
          ncol = 1)

f1 <- plot_grid(networks, 
                get_legend(clust_graph_mn + theme(legend.position = 'right')),
                f1b + theme(legend.position = 'none'), labels = c('A', '', 'B'), 
                nrow = 1, rel_widths = c(2, 0.5, 2)) %>% 
  suppressWarnings()

ggsave(plot = f1, filename = 'figures/Fig1.png', width = 12, height = 6)
ggsave(plot = f1, filename = 'figures/Fig1.tiff', width = 12, height = 6)
```
...
ggsave(plot = fs3, filename = 'figures/FigS3.tiff', width = 7.5, height = 10.5)
...
ggsave(plot = f2, filename = 'figures/Fig2.tiff', width = 7, height = 7)
...
ggsave(plot = fs4, filename = 'figures/FigS4.tiff', width = 7, height = 7)
fs4
```
...
```{r}
mn_snv_dat_orig %>% 
  filter(ST %in% c('ST171') & pairwise_dist <= 5 &
...
f3d <- mn_snv_dat_orig %>% 
  ...
  rowwise() %>% 
  mutate(f38_loc = any(c(loc1, loc2) == 'F38'),
         loc1_new = ifelse(f38_loc, 
                           c(loc1, loc2)[c(loc1, loc2) == 'F38'],
                           paste(sort(c(loc1, loc2)))[1]),
         loc2_new = ifelse(f38_loc, 
                           c(loc1, loc2)[c(loc1, loc2) != 'F38'],
                           paste(sort(c(loc1, loc2)))[2]),
         loc1_new = ifelse(loc1_new %in% c('F38'), loc1_new, 'Other')) %>% #ifelse(loc2_new %in% c('F38', 'F55', 'F34', 'F46'), loc2_new, 'Other')) %>%
  filter(loc1_new == 'F38') %>% 
  ungroup() %>%
  ...
  mutate(tot = sum(count),
         pct = paste0(round(count/tot*100), '%')) %>% 
  arrange(count/tot) %>% 
  mutate(loc2_new = factor(loc2_new, levels = unique(loc2_new))) %>% 
...
f3e <- bind_cols(inds=names(deg), 'Degree'=deg, 'Strength'=stre, 'Betweenness'=betw, 'Closeness'=clos) %>% 
  ...
  labs(x = 'Facility degree centrality based on patient transfers', y = 'Number of facilities') +
...
f3 <- plot_grid(f3a, 
          plot_grid(f3b + theme(legend.position = 'none'), 
                    f3c + theme(legend.position = 'none',
                                axis.text.x = element_text(size = 5)),
          nrow = 1, rel_widths = c(0.5, 1), labels = c('B', 'C')),
          plot_grid(f3d + 
                      theme(legend.position = 'left'),
          f3e, 
          labels = c( 'D', 'E'), rel_widths = c(1, 1, 1), nrow = 1, align = 'h', axis = 'trbl'),
          ncol = 1, labels = c('A', '', ''), rel_heights = c(1,1,1))

ggsave(plot = f3, filename = 'figures/Fig3.png', width = 7, height = 8)
ggsave(plot = f3, filename = 'figures/Fig3.tiff', width = 7, height = 8)
f3
```
...
hm_dat <- prior_hosps_tn %>% 
  ...
  left_join(facil_st %>% 
              mutate(ST = ifelse(ST %in% c('E. hormaechei ST171','E. hormaechei ST114',
                                 ...
                                 'ST258', 'ST307', 'ST114', 'ST171'),
                       ST, 'Other'))) %>% 
  left_join(cluster_info %>% rename(Sample_ID = isolate_no) %>% select(-ST)) %>% 
  filter(!is.na(Facility) & !is.na(plasmid_cluster)) %>% 
  group_by(Facility, ST) %>% 
  mutate(facil_st_count = n()) %>%
  group_by(plasmid_cluster, Facility, ST, facil_st_count) %>% 
  summarize(count = n()) %>% 
  mutate(Facility = factor(Facility, levels = rev(tn_hm$tree_col$labels[tn_hm$tree_col$order]) %>% gsub('TDH.', 'TDH-', .) %>% gsub('LONG.', 'LONG-', .) %>% gsub('VA.', 'VA-', .) %>% gsub('\\.', ' ', .)),
         ...
         ST = gsub('E. hormaechei', 'KPC-Eh', ST),
         ST = gsub('K. pneumoniae', 'KPC-Kp', ST),
         ST = ifelse(ST == 'KPC-Eh ST171', 'Other', ST),
         ST = factor(ST, levels = rev(c('KPC-Kp ST258', 'KPC-Eh ST114', 'KPC-Kp ST307', 'Other'))),
) %>% 
  group_by(Facility) %>% 
  mutate(dominant_st = case_when(is.infinite(max(facil_st_count[ST != 'Other'])) ~ 'Other\nmost common',
                         max(facil_st_count[ST == 'KPC-Kp ST258']) == max(facil_st_count[ST != 'Other']) ~ 'KPC-Kp ST258\nmost common',
                         max(facil_st_count[ST == 'KPC-Kp ST307']) == max(facil_st_count[ST != 'Other']) ~ 'KPC-Kp ST307\nmost common',
                         max(facil_st_count[ST == 'KPC-Eh ST114']) == max(facil_st_count[ST != 'Other']) ~ 'KPC-Eh ST114\nmost common',
                         TRUE ~ 'Other\nmost common'),
         dominant_st = factor(dominant_st, levels = c('KPC-Kp ST258\nmost common', 'KPC-Kp ST307\nmost common',
                                                      'KPC-Eh ST114\nmost common', 'Other\nmost common'))) %>% 
  # filter(!is.na(plasmid_cluster)) %>% 
  group_by(plasmid_cluster) %>% 
  mutate(plasmid_cluster = ifelse(sum(count) >= 10, plasmid_cluster, 'Other\nplasmid')) 

tn_facil_hm <- hm_dat %>% 
  ggplot(aes(x = Facility, y = ST, fill = count)) +
  facet_grid(plasmid_cluster~., scales = 'free_x', space = 'free_x') +
  geom_tile() +
  geom_vline(xintercept = 7+0.5, col = 'black', size = 1) +
  # geom_rect(aes(xmin = 0+0.5, xmax = 7+0.5, ymin = 0+0.5, ymax = 4+0.5), fill = NA, col = 'black', size = 1, linetype = 'dotted') +
...
```{r}
tn_facils_st <- facil_st %>% 
  filter(State == 'TN') %>% 
              mutate(ST = ifelse(ST %in% c('E. hormaechei ST114',
                                 'K. pneumoniae ST258','K. pneumoniae ST307', 
                                 'E. cloacae ST114'),
                       ST, 'Other')) %>% 
  left_join(cluster_info %>% rename(Sample_ID = isolate_no) %>% select(-ST)) %>% 
  filter(!is.na(Facility_ID)) %>% 
  group_by(Facility_ID, ST) %>% 
  summarize(facil_st_count = n()) %>%
  mutate(ST = gsub('E. hormaechei', 'KPC-Eh', ST),
         ST = gsub('K. pneumoniae', 'KPC-Kp', ST),
         ST = ifelse(ST == 'KPC-Eh ST171', 'Other', ST),
         ST = factor(ST, levels = rev(c('KPC-Kp ST258', 'KPC-Eh ST114', 'KPC-Kp ST307', 'Other'))),
) %>% 
  ...
  mutate(dominant_st = case_when(is.infinite(max(facil_st_count[ST != 'Other'])) ~ 'Other\nmost common',
         ...
         dominant_st = factor(dominant_st, levels = c('KPC-Kp ST258\nmost common', 'KPC-Kp ST307\nmost common',
                                                      'KPC-Eh ST114\nmost common', 'Other\nmost common'))) %>% 
  group_by(Facility_ID, dominant_st) %>% 
  summarize(n_isolates = sum(facil_st_count)) %>% 
  ungroup() %>% 
  arrange(Facility_ID) %>% 
  mutate(index = as.numeric(factor(Facility_ID)))
```


```{r}
graph_tn <- tbl_graph(nodes = tn_facils_st %>% 
                        mutate(dominant_st = gsub('\nmost common', '', dominant_st)), 
                      # node_key = 'Facility_ID',
                      edges = tn_trans_net_sub %>% 
                        filter(!grepl('I', source_facil) & !grepl('I', dest_facil) & n_transfers > 1) %>% 
                        left_join(tn_facils_st %>% rename(source_facil = Facility_ID, from = index) %>%
                                    select(-c(dominant_st, n_isolates))) %>% 
                        left_join(tn_facils_st %>% rename(dest_facil = Facility_ID, to = index) %>%
                                    select(-c(dominant_st, n_isolates))),
                      directed = TRUE) %>%
  induced.subgraph(igraph::degree(.) > 0)

st_cols_short <- st_cols
names(st_cols_short) <- gsub('K. pneumoniae', 'KPC-Kp', names(st_cols_short))
names(st_cols_short) <- gsub('E. hormaechei', 'KPC-Eh', names(st_cols_short))

tn_network <- graph_tn %>% 
  ggraph(layout = 'kk') + 
  geom_edge_link(aes(color = n_transfers, alpha = n_transfers), arrow = arrow(length = unit(3, 'mm'))) + 
  geom_node_point(aes(color = dominant_st, size = n_isolates)) + 
  # scale_edge_color_viridis(direction = -1, option = 'E') +
  scale_edge_color_continuous(high = '#00204c', low = '#ffe945') +
  scale_color_manual(values = st_cols_short) +
...
tn_facil_transfers <- tn_trans_net_sub %>% 
  left_join(tn_facils_st %>% mutate(source_of_interest = dominant_st %in% c('KPC-Kp ST307\nmost common', 'KPC-Eh ST114\nmost common')) %>% 
              select(Facility_ID, source_of_interest), 
            by = c(source_facil = 'Facility_ID')) %>% 
  left_join(tn_facils_st %>% mutate(dest_of_interest = dominant_st %in% c('KPC-Kp ST307\nmost common', 'KPC-Eh ST114\nmost common')) %>% 
...
ggsave(filename = 'figures/Fig4.tiff', plot = f4, width = 12, height = 5)
f4
```
